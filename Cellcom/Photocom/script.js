...
// Supabase initialization (client-side)
const SUPABASE_URL = 'https://nlmhellbkwswweutmeda.supabase.co';
// Use the anon/public key for client-side operations (reads/inserts only)
const SUPABASE_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im5sbWhlbGxia3dzd3dldXRtZWRhIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODMyMjQ4MzAsImV4cCI6MjA5ODgwMDgzMH0.4vNgzYYwRGGw7iIsESQsfnHMkoHZGvyEl5mE4Hahqao';
let supabaseClient = null;
if (typeof supabase !== 'undefined' && supabase.createClient) {
  supabaseClient = supabase.createClient(SUPABASE_URL, SUPABASE_KEY);
} else {
  // If the Supabase UMD hasn't loaded yet, expose an initializer
...
const LS = {
  get: (key) => {
    try {
      // For certain content keys, prefer teacher-scoped storage when a teacher is signed in
      const contentKeys = ['comics','quizzes','parts','games'];
      if (currentUser && currentUser.role === 'teacher' && contentKeys.includes(key)) {
        const td = JSON.parse(localStorage.getItem(getTeacherDataKey(currentUser.id)) || '{}') || {};
        return td[key] || null;
      }
  ...
  set: (key, value) => {
    try {
      const contentKeys = ['comics','quizzes','parts','games'];
      if (currentUser && currentUser.role === 'teacher' && contentKeys.includes(key)) {
        const tdKey = getTeacherDataKey(currentUser.id);
        const td = JSON.parse(localStorage.getItem(tdKey) || '{}') || {};
        td[key] = value;
        localStorage.setItem(tdKey, JSON.stringify(td));
        return;
      }
...
// Basic accessibility helpers and keyboard navigation
const initAccessibility = () => {
  // Add aria-labels for close buttons
  ...
  document.querySelectorAll('button').forEach(b => {
    if (!b.getAttribute('aria-label') && b.innerText.trim() === '' && b.querySelector('i')) {
      ...
      const name = ico && ico.getAttribute('data-lucide') ? ico.getAttribute('data-lucide') : 'button';
      b.setAttribute('aria-label', name);
    }
  });

  // Add keyboard navigation for jigsaw buttons
  document.querySelectorAll('#gp-jigsaw-container button').forEach(btn => {
    if (!btn.hasAttribute('tabindex')) btn.tabIndex = 0;
  });

  // Keyboard navigation for comic reader, fullscreen image modal, and games
  document.addEventListener('keydown', (e) => {
    const comicScreen = document.getElementById('sc-comic-reader');
    const fullscreenModal = document.getElementById('fullscreen-image-modal');
    const gamePlayer = document.getElementById('sc-game-player');
    
    if (comicScreen && !comicScreen.classList.contains('hidden')) {
      if (e.key === 'ArrowLeft') prevComicPage();
      if (e.key === 'ArrowRight') nextComicPage();
      if (e.key === 'Escape') exitComicReader();
      if (e.key === '+' || e.key === '=') zoomInComic();
      if (e.key === '-') zoomOutComic();
    }
    if (fullscreenModal && !fullscreenModal.classList.contains('hidden')) {
      if (e.key === 'ArrowLeft') fullscreenPrevComic();
      if (e.key === 'ArrowRight') fullscreenNextComic();
      if (e.key === 'Escape') closeFullscreenImageModal();
    }
    
    // Jigsaw puzzle keyboard shortcuts
    if (gamePlayer && !gamePlayer.classList.contains('hidden')) {
      const jigsawContainer = document.getElementById('gp-jigsaw-container');
      if (jigsawContainer && jigsawContainer.style.display !== 'none') {
        if (e.key === 's' || e.key === 'S') jigsawShuffle();
        if (e.key === 'h' || e.key === 'H') jigsawHint();
        if (e.key === 'p' || e.key === 'P') jigsawTogglePreview();
        if (e.key === 'r' || e.key === 'R') jigsawReset();
      }
    }
  });
};
...
const launchConfetti = () => {
  const colors = ['#7aa2f7', '#10b981', '#f59e0b', '#8b5cf6', '#f43f5e', '#38bdf8'];
...
let populateDashboard = () => {
  ...
  if (currentUser.role === 'teacher') {
    ...
    if (navbarName) navbarName.textContent = currentUser.fullName || 'Teacher';
    ...
    if (sideImg) {
      if (currentUser.photoURL) {
        ...
        const roleIcon = currentUser.role === 'teacher' ? 'graduation-cap' : 'user';
        sideImg.innerHTML = `<i data-lucide="${roleIcon}" style="width:20px;height:20px;color:var(--leaf);"></i>`;
        refreshLucide();
      }
    }

    // Navbar avatar (profile chip)
    const navbarAvatar = document.querySelector('.profile-avatar');
    if (navbarAvatar) {
      if (currentUser.photoURL) {
        navbarAvatar.innerHTML = `<img src="${currentUser.photoURL}" class="nav-avatar-img" alt="profile">`;
      } else {
        const roleIcon = currentUser.role === 'teacher' ? 'graduation-cap' : 'user';
        navbarAvatar.innerHTML = `<i data-lucide="${roleIcon}" class="nav-avatar-icon"></i>`;
        refreshLucide();
      }
    }
  } else {
...
const toggleMobileSidebar = () => {
  ...
  if (!sidebar) return;
  // Ensure sidebar is set up for mobile drawer
  sidebar.classList.add('mobile-drawer');
  const overlayId = 'mobileSidebarOverlay';
...
const handleTeacherSignin = () => {
  (async () => {
    ...
    try {
      ...
      // Sync teacher content from remote DB into local teacherData
      syncTeacherDataFromRemote();
      closeModal('teacherSigninModal');
      showScreen('sc-tdash');
      setTeacherTab('home');
      populateDashboard();
      startRoomSync();
      toast('Welcome back, ' + (currentUser.full_name || currentUser.fullName || 'Teacher') + '!', 'ok');
    } catch (e) {
...
const handleStudentSignin = () => {
  (async () => {
    ...
    try {
      ...
      // Check if student is blocked (if field exists)
      if (data.blocked) {
      ...
      if (studentData.currentRoomCode) {
        let roomData = getRoomData(studentData.currentRoomCode);
        if (!roomData && supabaseClient) {
          roomData = await fetchRoomDataRemote(studentData.currentRoomCode);
        }
        if (roomData && roomData.active) {
          showScreen('sc-sdash');
          setStudentTab('home');
          populateDashboard();
          if (isStudentUser(currentUser)) renderStudentHomeDashboard();
          startRoomSync();
          toast('Welcome back, ' + (currentUser.first_name || currentUser.firstName) + '!', 'ok');
          return;
        } else {
      ...
      toast('Welcome back, ' + (currentUser.first_name || currentUser.firstName) + '! Please join the room.', 'info');
    } catch (e) {
      console.warn('Sign in error', e);
      toast('Sign in failed', 'warn');
    }
...
const handleStudentJoinRoomFromModal = async () => {
  const codeInput = document.getElementById('studentJoinRoomCodeInput');
  ...
  let roomData = getRoomData(code);
  if (!roomData && supabaseClient) {
    roomData = await fetchRoomDataRemote(code);
  }
  if (!roomData || !roomData.active) {
...
const handleTeacherRegister = () => {
  ...
  const newTeacher = {
    // local UUID id to match DB UUID primary key
    id: genUuid(),
    fullName,
    username,
    email,
  ...
  // Insert teacher into Supabase (only) and do not store full user list locally
  (async () => {
    try {
      // Send `id` as UUID because DB uses UUID primary key
      const { data, error } = await supabaseClient.from('teachers').insert([{ id: newTeacher.id, full_name: newTeacher.fullName, username: newTeacher.username, email: newTeacher.email, password: newTeacher.password, photo_url: newTeacher.photoURL, created_at: newTeacher.createdAt }]).select().single();
      if (error) {
        console.warn('Supabase teacher insert error:', error);
        toast('Supabase save failed: ' + (error.message || JSON.stringify(error)), 'warn');
        return;
      }
      // Update local record with DB-returned id (string UUID)
      if (data && data.id) {
        newTeacher.id = String(data.id);
      }
      // Persist to local teachers list for offline state
      const teachersList = LS.get('teachers') || [];
      teachersList.push(newTeacher);
      LS.set('teachers', teachersList);
      toast('Registration successful! Please sign in.', 'ok');
      closeModal('teacherRegisterModal');
      openRoleModal('signin');
      selectRole('teacher');
    } catch (e) {
      console.warn('Supabase teacher save failed', e);
      toast('Registration failed', 'warn');
    }
  })();
};
...
const handleStudentRegister = () => {
  ...
  // Insert student into Supabase (only) and do not store full user list locally
  (async () => {
    try {
      // Send `id` as UUID because DB uses UUID primary key
      const { data, error } = await supabaseClient.from('students').insert([{ id: newStudent.id, first_name: newStudent.firstName, last_name: newStudent.lastName, grade_level: newStudent.gradeLevel, section: newStudent.section, username: newStudent.username, password: newStudent.password, photo_url: newStudent.photoURL, created_at: newStudent.createdAt }]).select().single();
      if (error) {
        console.warn('Supabase student insert error:', error);
        toast('Supabase save failed: ' + (error.message || JSON.stringify(error)), 'warn');
        return;
      }
      // Update local record with DB-returned id (string UUID)
      if (data && data.id) {
        newStudent.id = String(data.id);
      }
      // Persist to local students list for offline state
      const studentsList = LS.get('students') || [];
      studentsList.push(newStudent);
      LS.set('students', studentsList);
      toast('Registration successful! Please sign in.', 'ok');
      closeModal('studentRegisterModal');
      openRoleModal('signin');
      selectRole('student');
    } catch (e) {
      console.warn('Supabase student save failed', e);
      toast('Registration failed', 'warn');
    }
  })();
};
...
const openUserProfile = () => {
  ...
  const avImg = `
    <div class="profile-pic-area" id="profilePicArea">
      <div class="profile-pic-container" id="profilePicContainer" title="Profile photo">
        ${currentUser.photoURL
          ? `<img src="${currentUser.photoURL}" class="profile-pic-img" alt="profile photo">`
          : `<div class="profile-icon-fallback"><i data-lucide="${currentUser.role === 'teacher' ? 'graduation-cap' : 'user'}" style="width:40px;height:40px;color:var(--leaf);"></i></div>`
        }
      ...
      <div class="profile-pic-actions" id="profilePicActions">
        <button class="btn btn-icon" id="profileEditBtn" aria-label="Edit avatar"><i data-lucide="edit-2"></i></button>
        <button class="btn btn-icon btn-danger" id="profileRemoveBtn" aria-label="Remove avatar"><i data-lucide="trash-2"></i></button>
      </div>

      <input type="file" id="profile-pic-uploader" accept="image/*" style="display:none;" onchange="handleProfilePicChange(this)">

      <div class="profile-upload-progress hidden" id="profileUploadProgress">
        <div class="progress-bar" id="profileUploadProgressBar" style="width:0%"></div>
      </div>
    </div>
  `;
  ...
  // After rendering, wire upload handlers and buttons
  try { setupProfileImageHandlers(); } catch (e) { console.warn('setupProfileImageHandlers failed', e); }
  modal.classList.remove('hidden');
  refreshLucide();
};
...
const toggleProfileDropdown = (event) => {
  ...
  const menu = document.getElementById('navbarProfileDropdown');
  if (!menu) return;
...
document.addEventListener('click', (event) => {
  const menu = document.getElementById('navbarProfileDropdown');
  const btn = document.getElementById('navbarProfileBtn');
  if (!menu || !btn) return;
...
// Numeric temp id left for other usages (not used for teacher/student ids)
const genTempId = () => -Math.floor(Date.now());

const getTeacherDataKey = (teacherId) => `teacherData_${teacherId}`;
// Remote room persistence requires a Supabase table named "rooms" with a unique code column.
const ROOMS_TABLE = 'rooms';

const normalizeRoomDataForRemote = (code, data) => {
  const payload = {
    code: code,
    room_code: code,
  ...
  if (Array.isArray(data.joinedStudents)) {
    payload.joined_students = data.joinedStudents;
  }
  return payload;
};

// Remote sync helpers for teacher-scoped content (comics, quizzes, parts, games)
const upsertTeacherRecord = async (table, record) => {
  if (!supabaseClient || !currentUser || currentUser.role !== 'teacher') return null;
  try {
    const payload = { ...record, teacher_id: currentUser.id };
    const { data, error } = await supabaseClient.from(table).upsert([payload], { onConflict: 'id' }).select().single();
    if (error) {
      console.warn('Supabase upsert error', table, error);
      return null;
    }
    return data;
  } catch (e) {
...
const deleteTeacherRecord = async (table, id) => {
  if (!supabaseClient || !currentUser || currentUser.role !== 'teacher') return null;
  try {
    const { data, error } = await supabaseClient.from(table).delete().eq('id', id);
    if (error) console.warn('Supabase delete error', table, error);
    return data;
  } catch (e) { console.warn('Supabase delete exception', e); return null; }
};

const syncTeacherDataFromRemote = async () => {
  if (!supabaseClient || !currentUser || currentUser.role !== 'teacher') return;
  try {
    const teacherId = currentUser.id;
    const teacherData = LS.get(getTeacherDataKey(teacherId)) || {};
    // tables to sync
    const tables = ['comics','quizzes','parts','games'];
    for (const table of tables) {
      const { data, error } = await supabaseClient.from(table).select('*').eq('teacher_id', teacherId);
      if (error) {
        console.warn('Supabase fetch error', table, error);
        continue;
      }
      if (!Array.isArray(data)) continue;
      // Normalize rows into local shape
      teacherData[table] = data.map(r => {
        return {
          id: r.id,
          title: r.title || r.name || '',
          description: r.description || r.desc || '',
          partId: r.part_id || r.partId || null,
          locked: !!r.locked,
          unlockAt: r.unlock_at || r.unlockAt || '',
          pages: r.pages || r.content || [],
    ...
    // Persist merged teacherData
    localStorage.setItem(getTeacherDataKey(teacherId), JSON.stringify(teacherData));
    // Refresh UI
    renderParts();
    renderComics();
    renderQuizzes && renderQuizzes();
  } catch (e) {
    console.warn('syncTeacherDataFromRemote failed', e);
  }
};

const fetchRoomDataRemote = async (code) => {
  if (!supabaseClient || !code) return null;
  try {
    const columnsToTry = ['room_code', 'code'];
    let result = null;
    for (const col of columnsToTry) {
      ...
      if (error && error.message && error.message.toLowerCase().includes('could not find')) {
        continue;
      }
      if (error) {
        console.warn('fetchRoomDataRemote error for', col, error);
        return null;
      }
    }
    ...
    const room = {
      ...
      joinedStudents: Array.isArray(data.joined_students) ? data.joined_students : [],
      chatMessages: Array.isArray(data.chat_messages) ? data.chat_messages : []
    };
    const rooms = LS.get('rooms') || {};
    rooms[code] = room;
    LS.set('rooms', rooms);
    ...
    console.warn('fetchRoomDataRemote error', err);
    return null;
  }
};

const saveRoomDataRemote = async (code, data) => {
  if (!code) return;
  try {
    const payload = normalizeRoomDataForRemote(code, data);

    // Prefer server-side persistence which uses the Supabase service role key (bypasses RLS)
    let jr = null;
    try {
      const resp = await fetch('/api/save-room', {
        method: 'POST',
      ...
      if (resp.ok && jr && jr.ok) {
        toast('Room persisted via server', 'ok');
        return;
      }
      // If server responded but with an error, log and fall through to client attempt
      console.warn('server save-room failed', jr);
    } catch (e) {
      console.warn('server save-room network error', e);
    }

    // Do NOT attempt client-side upsert as anon key will often be blocked by RLS.
    // Instead log a warning so deployer can configure the server-side
    // `SUPABASE_SERVICE_ROLE_KEY` environment variable (used by `/api/save-room`).
    console.warn('server save-room failed or not available; skipping client upsert', jr);
    // Respect user preference: show or suppress remote persist warnings
    const showRemoteWarn = !!LS.get('showRemotePersistWarnings');
    if (showRemoteWarn) {
      toast('Could not persist room remotely. Ensure the server API is configured with SUPABASE_SERVICE_ROLE_KEY.', 'warn');
    } else {
      // Keep it quiet in the UI; still log for debugging
      console.debug('Remote persist skipped (warning suppressed).');
    }
    return;
  } catch (err) {
    console.warn('saveRoomDataRemote error', err);
    toast('Supabase save failed: ' + formatErrorMessage(err), 'warn');
  }
};
...
const getTeacherRooms = (onlyActive = true) => {
  if (!currentUser || currentUser.role !== 'teacher') return [];
  const rooms = LS.get('rooms') || {};
  return Object.entries(rooms)
    .filter(([code, room]) => room && room.teacherId === currentUser.id && (!onlyActive || room.active))
    .map(([code, room]) => ({ code, ...room }))
    .sort((a, b) => (b.createdAt || 0) - (a.createdAt || 0));
};

const setTeacherCurrentRoom = (code) => {
  if (!currentUser || currentUser.role !== 'teacher') return null;
  const teacherData = LS.get(getTeacherDataKey(currentUser.id)) || {};
  teacherData.currentRoomCode = code || null;
...
const openTeacherRoom = (code, event) => {
  if (event) event.stopPropagation();
  const currentCode = getCurrentRoomCode();
  const activeRooms = getTeacherRooms(true);
  if (!code && currentCode) {
    setTeacherCurrentRoom(currentCode);
    return;
  }
  if (!code && activeRooms.length) {
    setTeacherCurrentRoom(activeRooms[0].code);
    return;
  }
  if (!code) {
    toast('No active teacher room found', 'warn');
    return;
  }
  const selected = getTeacherRooms(true).find(r => r.code === code);
  if (!selected) {
    toast('Selected room not found or is inactive', 'warn');
    return;
  }
  setTeacherCurrentRoom(code);
};

const removeTeacherRoom = async (event, code = null) => {
  if (event) event.stopPropagation();
  if (!currentUser || currentUser.role !== 'teacher') return;
  const roomCode = code || getCurrentRoomCode();
  if (!roomCode) {
    toast('No active room to remove', 'warn');
    return;
  }
  const confirmed = await openConfirmModal(`Remove room ${roomCode}? This will deactivate the room and clear it from your current room.`);
  if (!confirmed) return;

  const rooms = LS.get('rooms') || {};
  if (!rooms[roomCode]) {
    toast('Room not found', 'warn');
    return;
  }

  rooms[roomCode].active = false;
  LS.set('rooms', rooms);
  saveRoomData(roomCode, rooms[roomCode]);

  const teacherData = LS.get(getTeacherDataKey(currentUser.id)) || {};
  if (teacherData.currentRoomCode === roomCode) {
    const activeRooms = getTeacherRooms(true).filter(r => r.code !== roomCode);
    if (activeRooms.length) {
      teacherData.currentRoomCode = activeRooms[0].code;
    } else {
      teacherData.currentRoomCode = null;
    }
    LS.set(getTeacherDataKey(currentUser.id), teacherData);
  }
  ...
  renderTeacherRoomDropdown();
  refreshTeacherRoomUI();
  toast('Room removed successfully', 'ok');
};

const renderTeacherRoomDropdown = () => {
  const menu = document.getElementById('teacherRoomDropdownMenu');
  if (!menu) return;

  const activeRooms = getTeacherRooms(true);
  const currentCode = getCurrentRoomCode();
  const items = activeRooms.length ? activeRooms.map(room => {
    return `<button class="sidebar-sub-btn${room.code === currentCode ? ' on' : ''}" onclick="openTeacherRoom('${room.code}', event)">
              <i data-lucide="hash" style="width: 14px; height: 14px;"></i>
              <span>${room.code}${room.code === currentCode ? ' (active)' : ''}</span>
            </button>`;
  }) : [`<div class="sidebar-sub-placeholder">No existing rooms yet. Create one to begin.</div>`];

  const createBtn = `<button class="sidebar-sub-btn" onclick="generateNewRoomCode(); event.stopPropagation();">
            <i data-lucide="plus-circle" style="width: 14px; height: 14px;"></i>
            <span>Add Another Room</span>
          </button>`;

  const removeBtn = `<button class="sidebar-sub-btn" onclick="removeTeacherRoom(event);" style="color: var(--accent-rose);">
            <i data-lucide="trash-2" style="width: 14px; height: 14px;"></i>
            <span>Remove Current Room</span>
          </button>`;

  menu.innerHTML = `${items.join('')}${createBtn}${removeBtn}`;
  const currentText = document.getElementById('existingTeacherRoomText');
  if (currentText) {
    currentText.textContent = currentCode ? `Open Current Room (${currentCode})` : 'Open Current Room';
  }
  refreshLucide();
};
...
const refreshTeacherRoomUI = () => {
  ...
  const roomBtn = document.getElementById('teacherRoomCurrentBtn');

  if (!currentCode) {
    if (codeEl) codeEl.textContent = 'No active room';
    if (lockBtn) lockBtn.innerHTML = '<i data-lucide="lock"></i> No room';
    if (sgridEl) sgridEl.innerHTML = `<div class="empty-state"><div class="empty-state-icon">👥</div>No active room</div>`;
    if (chatMsgsEl) chatMsgsEl.innerHTML = `<div class="empty-state"><div class="empty-state-icon">💬</div>No active room yet</div>`;
    if (roomBtn) roomBtn.innerHTML = 'Open Current Room';
    renderTeacherRoomDropdown();
    refreshLucide();
    return;
  }
...
const generateNewRoomCode = async (partId = null) => {
  if (!currentUser || currentUser.role !== 'teacher') return null;

  // If teacher already has an active code, ask for confirmation
  const teacherData = LS.get(getTeacherDataKey(currentUser.id)) || {};
  if (teacherData.currentRoomCode) {
    const confirmed = await openConfirmModal('Create a new room while keeping existing active rooms available for switching. Continue?');
    if (!confirmed) return null;
  }

  const newCode = makeRandomCode(6);

  // Initialize rooms and keep previous teacher rooms active so teachers can manage multiple sessions.
  const rooms = LS.get('rooms') || {};
  ...
  rooms[newCode] = roomObj;

  // Save teacher's current code and persist
  teacherData.currentRoomCode = newCode;
  LS.set(getTeacherDataKey(currentUser.id), teacherData);

  // Persist new room (local + remote via saveRoomData)
  saveRoomData(newCode, roomObj);
...
const addNewPart = () => {
  ...
  if (partModalEditId) {
    ...
    parts.push(newPart);
    LS.set('parts', parts);
    // Remote insert/upsert
    upsertTeacherRecord('parts', newPart).then(remote => {
      if (remote && remote.id && remote.id !== newPart.id) {
        const list = getStoredParts();
        const i = list.findIndex(p => p.id === newPart.id);
        if (i !== -1) { list[i].id = remote.id; LS.set('parts', list); }
      }
    });
...
const deletePart = (partId) => {
  ...
  openConfirmModal('Are you sure you want to delete this part? All linked contents will remain but lose their part link.')
    .then(confirmed => {
      ...
      // Remote delete
      deleteTeacherRecord('parts', partId);
...
const togglePartLock = (partId) => {
  ...
  if (part) {
    ...
    LS.set('parts', parts);
    // Remote update
    upsertTeacherRecord('parts', part);
...
const openGameModalWithType = (type, selectedPartId = null, gameId = null) => {
  ...
  partSelect.innerHTML = parts.map((part, idx) => `<option value="${part.id}">${part.title || getPartOrdinalLabel(idx)}</option>`).join('');
...
const openGamePreview = (gameId) => {
  ...
  if (bodyEl) {
    ...
    // Leaderboard section
    const leaderboards = LS.get('game_leaderboards') || {};
    const gameLeaderboard = leaderboards[gameId] || [];
    let leaderboardHtml = '';
    if (gameLeaderboard.length > 0) {
      leaderboardHtml = `
        <div class="game-preview-box" style="margin-top:16px;">
          <div style="font-weight:700;font-size:0.95rem;color:var(--txth);margin-bottom:10px;">🏆 Leaderboard (Top 10)</div>
          <div style="display:flex;flex-direction:column;gap:8px;">
            ${gameLeaderboard.map((entry, idx) => {
              ...
              return `
                <div style="display:flex;justify-content:space-between;align-items:center;padding:8px 12px;background:var(--surf2);border-radius:8px;border:1px solid var(--bdr);">
                  <div style="display:flex;align-items:center;gap:10px;">
                    <div style="width:28px;height:28px;border-radius:50%;background:var(--surf3);display:flex;align-items:center;justify-content:center;font-weight:700;">${rankIcon}</div>
                    <div style="font-weight:600;color:var(--txth);">${entry.studentName}</div>
                  </div>
                  <div style="font-weight:700;color:var(--leaf);">${timeStr}</div>
                </div>
              `;
            }).join('')}
          </div>
...
const loadGamePlayer = (gameId) => {
  ...
  if (game.type === 'word-search') {
    ...
    const exitBtn = document.getElementById('gp-exit-btn');
    if (submitBtn) submitBtn.style.display = 'none';
    if (exitBtn) exitBtn.style.display = 'block';
  } else if (game.type === 'jigsaw') {
    renderJigsawGame(game);
    setGameTips('jigsaw');
    // For jigsaw, hide global submit/exit controls to simplify play on all devices
    const submitBtn = document.getElementById('gp-submit-btn');
    const exitBtn = document.getElementById('gp-exit-btn');
    if (submitBtn) submitBtn.style.display = 'none';
...
const completeWordSearch = () => {
  ...
  updateLeaderboard(
    ...
    currentUser.id,
    currentUser.name || 'Unknown Student',
    totalElapsed
  );
...
const renderJigsawGame = (game) => {
  ...
  // Game level metadata is stored in settings.levelDetails (created by saveGame())
  const levelDetails = game.settings?.levelDetails || [];
  const currentLevelMeta = levelDetails[currentGameLevelIndex] || {};

  const pieces = parseInt(currentLevelMeta.pieces || currentLevelMeta.pieces === 0 ? currentLevelMeta.pieces : (game.settings?.pieces || 9), 10) || 9;
  const imageUrl = (currentLevelMeta.image || '').trim() || (game.settings?.image || '').trim() || '';

  document.getElementById('gp-jigsaw-title').textContent = currentLevelMeta.title || game.title || 'Puzzle';
  document.getElementById('gp-jigsaw-description').textContent = currentLevelMeta.description || game.description || 'Complete this puzzle to continue.';

  // Update reference images
  const referenceImg = document.getElementById('gp-jigsaw-reference');
  if (referenceImg) referenceImg.src = imageUrl;
  const bgPreviewImg = document.getElementById('gp-jigsaw-bg-preview');
  if (bgPreviewImg) bgPreviewImg.src = imageUrl;
  ...
  // Find saved game state if exists
  let savedState = null;
  if (currentGameData && currentUser) {
    const gameAttempts = LS.get('game_attempts') || [];
    const attempt = gameAttempts.find(a => a.gameId === currentGameData.gameId && a.studentId === currentUser.id && !a.completed && a.gameState);
    if (attempt && attempt.gameState) {
      savedState = attempt.gameState;
    }
  }

  initJigsawGame(imageUrl, pieces, savedState);
  container.style.display = 'block';
};

const initJigsawGame = (imageUrl, numPieces, savedState = null) => {
  jigsawState = {
    pieces: [],
    boardSlots: [],
    currentPiece: null,
    ...
    hintsLeft: savedState?.hintsLeft ?? 3,
    moves: savedState?.moves ?? 0,
    startTime: Date.now(),
    timerInterval: null,
    previewVisible: savedState?.previewVisible ?? false,
    rows: 0,
    cols: 0,
    imageUrl: imageUrl
  };
  ...
  jigsawState.rows = Math.ceil(Math.sqrt(numPieces));
  jigsawState.cols = Math.ceil(numPieces / jigsawState.rows);

  const boardWrapper = boardContainer.parentElement;
  const maxBoardWidth = boardWrapper ? Math.min(boardWrapper.clientWidth, window.innerWidth - 80) : 320;
  // Make the puzzle larger and more touch-friendly across devices
  const maxBoardHeight = window.innerHeight * 0.72; // use more vertical space
  const maxDimension = Math.min(maxBoardWidth, maxBoardHeight);
  const pieceSize = Math.max(48, Math.min(140, Math.floor(maxDimension / Math.max(jigsawState.cols, jigsawState.rows))));
  boardContainer.style.width = `${jigsawState.cols * pieceSize}px`;
  boardContainer.style.height = `${jigsawState.rows * pieceSize}px`;

  const imageUrlSafe = imageUrl;
  const placedIndices = new Set(savedState?.piecesPlacedIndices || []);

  for (let i = 0; i < numPieces; i++) {
    const row = Math.floor(i / jigsawState.cols);
    const col = i % jigsawState.cols;
    const xPercent = jigsawState.cols > 1 ? (col / (jigsawState.cols - 1)) * 100 : 0;
    const yPercent = jigsawState.rows > 1 ? (row / (jigsawState.rows - 1)) * 100 : 0;
    ...
    piece.draggable = !placedIndices.has(i);
    piece.style.width = `${pieceSize}px`;
    piece.style.height = `${pieceSize}px`;
    piece.style.backgroundImage = `url(${imageUrlSafe})`;
    piece.style.backgroundSize = `${jigsawState.cols * 100}% ${jigsawState.rows * 100}%`;
    piece.style.backgroundPosition = `${xPercent}% ${yPercent}%`;
    piece.style.backgroundRepeat = 'no-repeat';
    piece.style.border = '2px solid var(--bdr)';
    piece.style.borderRadius = '8px';
    piece.style.cursor = placedIndices.has(i) ? 'default' : 'grab';
    ...
    slot.addEventListener('dragenter', handleJigsawDragEnter);
    slot.addEventListener('dragleave', handleJigsawDragLeave);
    slot.addEventListener('drop', handleJigsawDrop);

    boardContainer.appendChild(slot);
    jigsawState.boardSlots.push({ element: slot, index: i, filled: placedIndices.has(i) });
    ...
    if (placedIndices.has(i)) {
      ...
      jigsawState.pieces.push({ element: piece, index: i, placed: true });
  ...
  document.getElementById('gp-jigsaw-hints-left').textContent = jigsawState.hintsLeft;
  updateJigsawProgress();
  
  // Set preview visibility
  const previewContainer = document.getElementById('gp-jigsaw-preview-container');
  const previewButton = document.getElementById('gp-jigsaw-preview-btn');
  if (jigsawState.previewVisible) {
    previewContainer.style.display = 'block';
    previewButton.innerHTML = '<i data-lucide="eye-off" style="width:16px;height:16px;"></i> Hide Preview';
  } else {
    previewContainer.style.display = 'none';
    previewButton.innerHTML = '<i data-lucide="image" style="width:16px;height:16px;"></i> Show Preview';
  }
  
  if (!savedState) {
    shufflePieces();
  } else {
    // If we have saved state, shuffle only unplaced pieces
    const unplacedPieces = Array.from(piecesContainer.children);
    for (let i = unplacedPieces.length - 1; i > 0; i--) {
      const j = Math.floor(Math.random() * (i + 1));
      piecesContainer.insertBefore(unplacedPieces[j], unplacedPieces[i]);
    }
  }
...
const startJigsawTimer = () => {
  jigsawState.timerInterval = setInterval(() => {
    const elapsed = Math.floor((Date.now() - jigsawState.startTime) / 1000);
    const minutes = Math.floor(elapsed / 60);
    const seconds = elapsed % 60;
    const timeString = `${String(minutes).padStart(2, '0')}:${String(seconds).padStart(2, '0')}`;
    document.getElementById('gp-jigsaw-time').textContent = timeString;
  }, 1000);
};

const updateJigsawProgress = () => {
  const progressPercent = Math.floor((jigsawState.piecesPlaced / jigsawState.totalPieces) * 100);
  document.getElementById('gp-jigsaw-progress-fill').style.width = `${progressPercent}%`;
  document.getElementById('gp-jigsaw-progress-text').textContent = `${progressPercent}%`;
  document.getElementById('gp-jigsaw-moves').textContent = `${jigsawState.moves} moves`;
  document.getElementById('gp-jigsaw-pieces-left').textContent = jigsawState.totalPieces - jigsawState.piecesPlaced;
  document.getElementById('gp-jigsaw-piece-tracker').textContent = 
    `${jigsawState.totalPieces} pieces · ${jigsawState.piecesPlaced} placed · ${jigsawState.totalPieces - jigsawState.piecesPlaced} remaining`;
};
...
// New shuffle handler for UI button
const jigsawShuffle = () => {
  shufflePieces();
  toast('Pieces shuffled!', 'info');
};
...
const handleJigsawDrop = (e) => {
  ...
  // Auto-snap to nearby pieces if close
  let targetSlotIndex = slotIndex;
  if (pieceIndex !== slotIndex) {
    // Check adjacent slots for auto-snap
    const pieceRow = Math.floor(pieceIndex / jigsawState.cols);
    const pieceCol = pieceIndex % jigsawState.cols;
    const targetRow = Math.floor(slotIndex / jigsawState.cols);
    const targetCol = slotIndex % jigsawState.cols;
    
    // Check if within 1 row/col (auto-snap)
    if (Math.abs(pieceRow - targetRow) <= 1 && Math.abs(pieceCol - targetCol) <= 1) {
      targetSlotIndex = pieceIndex;
      // Find the correct slot
      const correctSlot = document.querySelector(`[data-slot-index="${pieceIndex}"]`);
      if (correctSlot && !jigsawState.boardSlots[pieceIndex].filled) {
        placePiece(correctSlot, jigsawState.currentPiece, pieceIndex);
        return;
      }
    }
  }
    ...
    if (slot) {
      ...
      if (pieceIndex !== slotIndex) {
        ...
        if (Math.abs(pieceRow - targetRow) <= 1 && Math.abs(pieceCol - targetCol) <= 1) {
          const correctSlot = document.querySelector(`[data-slot-index="${pieceIndex}"]`);
          if (correctSlot && !jigsawState.boardSlots[pieceIndex].filled) {
            placePiece(correctSlot, touchPiece, pieceIndex);
            return;
          }
        }
      }
...
const handleJigsawPieceClick = (e) => {
  const piece = e.currentTarget;
  if (!piece || piece.dataset && piece.dataset.pieceIndex === undefined) return;

  if (jigsawState.currentPiece && jigsawState.currentPiece !== piece) {
    jigsawState.currentPiece.classList.remove('jigsaw-piece-selected');
  }

  jigsawState.currentPiece = piece;
  piece.classList.add('jigsaw-piece-selected');
};

const handleJigsawSlotClick = (e) => {
  if (!jigsawState.currentPiece) return;
...
const placePiece = (slot, piece, index) => {
  ...
  piece.style.border = 'none';
  piece.classList.add('placed');
  piece.classList.remove('jigsaw-piece-selected');
  slot.appendChild(piece);
  
  if (jigsawState.currentPiece === piece) {
    jigsawState.currentPiece = null;
  }
  ...
  // Remove any hint pulse from this slot
  ...
  if (jigsawState.piecesPlaced === jigsawState.totalPieces) {
    // Stop timer
    if (jigsawState.timerInterval) {
      clearInterval(jigsawState.timerInterval);
    }
    
    // Add completion animation
    document.getElementById('gp-jigsaw-board').classList.add('jigsaw-complete');
...
// New jigsaw features
const jigsawHint = () => {
  if (jigsawState.hintsLeft <= 0) {
    toast('No hints left!', 'warn');
    return;
  }
  
  // Find an unplaced piece
  const unplaced = jigsawState.pieces.filter(p => !p.placed);
  if (unplaced.length === 0) {
    toast('All pieces placed!', 'info');
    return;
  }
  ...
  // Find the corresponding slot and pulse it
  const slot = document.querySelector(`[data-slot-index="${randomPiece.index}"]`);
  if (slot) {
    slot.classList.add('hint-pulse');
    toast('Check the pulsing slot!', 'info');
  ...
  jigsawState.hintsLeft--;
  document.getElementById('gp-jigsaw-hints-left').textContent = jigsawState.hintsLeft;
};

const jigsawTogglePreview = () => {
  jigsawState.previewVisible = !jigsawState.previewVisible;
  const previewContainer = document.getElementById('gp-jigsaw-preview-container');
  const button = document.getElementById('gp-jigsaw-preview-btn');
  
  if (jigsawState.previewVisible) {
    previewContainer.style.display = 'block';
    button.innerHTML = '<i data-lucide="eye-off" style="width:16px;height:16px;"></i> Hide Preview';
  } else {
    previewContainer.style.display = 'none';
    button.innerHTML = '<i data-lucide="image" style="width:16px;height:16px;"></i> Show Preview';
  }
  refreshLucide();
};

const jigsawReset = () => {
  if (confirm('Reset the puzzle? All progress will be lost!')) {
    initJigsawGame(jigsawState.imageUrl, jigsawState.totalPieces);
    toast('Puzzle reset!', 'info');
  }
};
...
const updateLeaderboard = (gameId, studentId, studentName, timeUsedSeconds) => {
  // Leaderboard structure: { gameId: [{ studentId, studentName, timeSeconds, completedAt }] }
  const leaderboards = LS.get('game_leaderboards') || {};
  ...
  const existingIndex = leaderboards[gameId].findIndex(e => e.studentId === studentId);
  ...
  if (existingIndex !== -1) {
    ...
    if (timeUsedSeconds < leaderboards[gameId][existingIndex].timeSeconds) {
      leaderboards[gameId][existingIndex] = newEntry;
    }
  } else {
    leaderboards[gameId].push(newEntry);
  }
  
  // Sort by fastest time (ascending)
  leaderboards[gameId].sort((a, b) => a.timeSeconds - b.timeSeconds);
  
  // Keep top 10
  if (leaderboards[gameId].length > 10) {
    leaderboards[gameId] = leaderboards[gameId].slice(0, 10);
  }
  
  LS.set('game_leaderboards', leaderboards);
};
...
const exitGamePlayer = () => {
  // Save game session with paused time and specific game state
  if (currentGameData && currentGameData.gameId && currentUser) {
    const gameAttempts = LS.get('game_attempts') || [];
    let attempt = gameAttempts.find(a => a.gameId === currentGameData.gameId && a.studentId === currentUser.id && !a.completed);
    ...
    if (currentGameData.type === 'jigsaw') {
      attempt.gameState = {
        piecesPlacedIndices: jigsawState.pieces.filter(p => p.placed).map(p => p.index),
        totalPieces: jigsawState.totalPieces,
        hintsLeft: jigsawState.hintsLeft,
        moves: jigsawState.moves,
        previewVisible: jigsawState.previewVisible
      };
    } else if (currentGameData.type === 'word-search') {
      attempt.gameState = {
        foundWords: wordSearchState?.foundWords || [],
        gridSize: wordSearchState?.gridSize || 10
      };
    } else if (currentGameData.type === 'cryptogram') {
      attempt.gameState = {
        userMap: cryptogramState?.userMap || {},
        hintsGiven: cryptogramState?.hintsGiven || 0
      };
    }
...
const openComicModal = (selectedPartId = null, comicId = null) => {
  ...
  cmPartSelect.innerHTML = parts.map((p, idx) => `<option value="${p.id}">${p.title || getPartOrdinalLabel(idx)}</option>`).join('');
  comicModalEditId = comicId || null;
  comicEditorSelectedCard = null;
  ...
  const previewPanel = document.getElementById('cm-preview-panel');
  const checklist = document.getElementById('cm-publish-checklist');
  if (editor) editor.innerHTML = '';
  if (previewPanel) previewPanel.innerHTML = '<div class="comic-preview-empty">Select a page to preview</div>';
  ...
  if (comicModalEditId) {
    ...
    if (editor) (existing.pages || []).forEach(page => addComicPageEditor(page));
    ...
    const restoredDraft = LS.get('comicDraftCurrent');
    if (restoredDraft && restoredDraft.pages && restoredDraft.pages.length) {
      restoreComicDraft(restoredDraft);
    } else {
      addComicPageEditor();
    }
    if (modalTitle) modalTitle.textContent = 'Create Comic';
    if (saveBtn) saveBtn.textContent = 'Add page';
  }

  renderComicPublishChecklist();
...
const addComicPageEditor = (data = null) => {
  ...
  const pageId = data?.id || genUuid();
  const card = document.createElement('div');
  card.className = 'comic-page-edit-card';
  card.dataset.pageId = pageId;
  card.dataset.displayMode = data?.displayMode || 'fit';
  card.dataset.rotateDeg = data?.rotateDeg || '0';
  card.dataset.flipX = data?.flipX || '0';
  card.dataset.flipY = data?.flipY || '0';
  card.innerHTML = `
    <div class="comic-page-card-top">
      <div class="comic-page-card-title">
        <span class="drag-handle" title="Drag to reorder"><i data-lucide="grip"></i></span>
        <strong>Page #${idx + 1}</strong>
      </div>
      <div class="comic-page-card-actions">
        <button class="btn btn-ghost btn-sm" type="button" onclick="duplicateComicPage(this)" title="Duplicate page"><i data-lucide="copy"></i></button>
        <button class="choice-delete-btn" type="button" onclick="confirmRemoveComicPage(this)" title="Remove page"><i data-lucide="trash-2"></i></button>
      </div>
    </div>
    <div class="comic-page-preview">
      <img class="cm-page-thumb" src="${data?.image || ''}" alt="Panel preview" ${data?.image ? '' : 'style="opacity:0.25;"'}>
      <div class="preview-meta">
        <div class="preview-meta-top">
          <input class="inp cm-page-img-name" placeholder="Image title (optional)" value="${escapeHtml(data?.imageName || '')}">
          <div class="comic-page-tools-dropdown">
            <button type="button" class="btn btn-icon comic-page-tools-dropdown-button" title="Page actions">
              <i data-lucide="more-vertical" style="width:18px;height:18px;"></i>
            </button>
            <div class="comic-page-tools-menu hidden">
              <button type="button" class="comic-page-tool-item" data-action="rotate">Rotate</button>
              <button type="button" class="comic-page-tool-item" data-action="flip-h">Flip horizontal</button>
              <button type="button" class="comic-page-tool-item" data-action="flip-v">Flip vertical</button>
              <div class="comic-page-tools-divider"></div>
              <button type="button" class="comic-page-tool-item" data-action="mode-fit">Fit</button>
              <button type="button" class="comic-page-tool-item" data-action="mode-fill">Fill</button>
              <button type="button" class="comic-page-tool-item" data-action="mode-stretch">Stretch</button>
            </div>
          </div>
        </div>
        <input type="hidden" class="cm-page-img-data" value="${escapeHtml(data?.image || '')}">
        <div style="display:flex;gap:8px;align-items:center;margin-top:8px;">
          <button type="button" class="btn btn-ghost cm-choose-file">Choose image</button>
          <small style="opacity:0.7;font-size:0.8rem;">or drop image here</small>
        </div>
        <input type="file" accept="image/*" class="cm-page-file-input" style="display:none;">
      </div>
    </div>
    <div class="comic-page-fields">
      <input class="inp cm-page-title" placeholder="Page title" value="${escapeHtml(data?.title || '')}">
      <textarea class="inp cm-page-caption" placeholder="Caption / dialogue" rows="2">${escapeHtml(data?.caption || data?.text || '')}</textarea>
    </div>`;
  container.appendChild(card);
  refreshLucide();
  enableComicPageReorder();

  const fileInput = card.querySelector('.cm-page-file-input');
  const chooseBtn = card.querySelector('.cm-choose-file');
  const thumb = card.querySelector('.cm-page-thumb');
  const previewWrap = card.querySelector('.comic-page-preview');

  const wireCard = () => {
    if (chooseBtn && fileInput) {
      chooseBtn.addEventListener('click', (e) => { e.stopPropagation(); fileInput.click(); });
      fileInput.addEventListener('change', () => handleComicPageFileInput(fileInput));
    }
    if (thumb) {
      thumb.addEventListener('click', (e) => { e.stopPropagation(); fileInput && fileInput.click(); });
    }
    if (previewWrap) {
      previewWrap.addEventListener('dragover', (e) => { e.preventDefault(); previewWrap.classList.add('dragover'); });
      previewWrap.addEventListener('dragleave', (e) => { e.preventDefault(); previewWrap.classList.remove('dragover'); });
      previewWrap.addEventListener('drop', (e) => {
        e.preventDefault(); previewWrap.classList.remove('dragover');
        const files = Array.from(e.dataTransfer.files || []).filter(f => f.type && f.type.startsWith('image/'));
        if (!files.length) { toast('Please drop an image file here.', 'warn'); return; }
        readFileIntoCard(files[0], card);
      });
    }
    const closeAllPageToolMenus = () => document.querySelectorAll('.comic-page-tools-menu').forEach(menu => menu.classList.add('hidden'));
    const dropdownBtn = card.querySelector('.comic-page-tools-dropdown-button');
    const toolMenu = card.querySelector('.comic-page-tools-menu');
    if (dropdownBtn && toolMenu) {
      dropdownBtn.addEventListener('click', (e) => {
        e.stopPropagation();
        const wasOpen = !toolMenu.classList.contains('hidden');
        closeAllPageToolMenus();
        if (wasOpen) {
          toolMenu.classList.add('hidden');
        } else {
          toolMenu.classList.remove('hidden');
          const rect = dropdownBtn.getBoundingClientRect();
          const menuWidth = toolMenu.offsetWidth || 180;
          const viewportLeft = 12;
          const viewportRight = window.innerWidth - 12;
          const preferredLeft = rect.right - menuWidth;
          const left = Math.min(Math.max(viewportLeft, preferredLeft), viewportRight - menuWidth);
          toolMenu.style.position = 'fixed';
          toolMenu.style.top = `${Math.min(window.innerHeight - toolMenu.offsetHeight - 12, rect.bottom + 8)}px`;
          toolMenu.style.left = `${left}px`;
          toolMenu.style.right = 'auto';
        }
      });
    }
    card.querySelectorAll('.comic-page-tool-item').forEach(btn => {
      btn.addEventListener('click', (e) => {
        e.stopPropagation();
        handleComicPageTool(btn, card);
        toolMenu?.classList.add('hidden');
      });
    });
    document.addEventListener('click', (e) => {
      if (!e.target.closest('.comic-page-tools-dropdown')) {
        toolMenu?.classList.add('hidden');
      }
    });
    card.querySelectorAll('.cm-page-title, .cm-page-caption, .cm-page-img-name').forEach(input => {
      input.addEventListener('input', () => { saveComicDraft(); renderComicPreviewFromCard(card); renderComicPublishChecklist(); });
    });
    card.addEventListener('click', (e) => {
      if (e.target.closest('.comic-page-card-actions') || e.target.closest('.comic-page-tools')) return;
      selectComicPageEditor(card);
    });
  };

  wireCard();
  if (data?.image) updateComicPagePreviewFromData(card, data.image, data.imageName || '');
  if (!comicEditorSelectedCard) selectComicPageEditor(card);
  saveComicDraft();
  return card;
};

const selectComicPageEditor = (card) => {
  const container = document.getElementById('cm-pages-editor');
  if (!container) return;
  container.querySelectorAll('.comic-page-edit-card').forEach(item => item.classList.toggle('selected', item === card));
  comicEditorSelectedCard = card;
  renderComicPreviewFromCard(card);
};

const renderComicPreviewFromCard = (card) => {
  const preview = document.getElementById('cm-preview-panel');
  if (!preview) return;
  if (!card) {
    preview.innerHTML = '<div class="comic-preview-empty">Select a page to preview</div>';
    return;
  }
  const imageData = card.querySelector('.cm-page-img-data')?.value || '';
  const title = card.querySelector('.cm-page-title')?.value || '';
  const caption = card.querySelector('.cm-page-caption')?.value || '';
  const displayMode = card.dataset.displayMode || 'fit';
  const style = `object-fit:${displayMode === 'stretch' ? 'fill' : displayMode === 'fill' ? 'cover' : 'contain'};`;
  preview.innerHTML = `
    <div style="width:100%;display:flex;flex-direction:column;gap:14px;">
      ${imageData ? `<img class="comic-preview-image" src="${imageData}" alt="${escapeHtml(title || caption || 'Comic page preview')}" style="${style}" onclick="openFullscreenImageModal('${imageData}')">` : '<div class="comic-preview-empty">Add an image to preview this page</div>'}
      <div class="comic-preview-copy">
        ${title ? `<div class="comic-preview-title">${escapeHtml(title)}</div>` : ''}
        ${caption ? `<div class="comic-preview-caption">${escapeHtml(caption)}</div>` : ''}
      </div>
    </div>
  `;
};

const getComicPageDataFromCard = (card, idx) => {
  const image = card.querySelector('.cm-page-img-data')?.value || '';
  const imageName = card.querySelector('.cm-page-img-name')?.value.trim() || '';
  const title = card.querySelector('.cm-page-title')?.value.trim() || '';
  const caption = card.querySelector('.cm-page-caption')?.value.trim() || '';
  return {
    id: card.dataset.pageId || `page_${idx}`,
    image,
    imageName,
    title,
    caption,
    text: caption,
    displayMode: card.dataset.displayMode || 'fit',
    rotateDeg: Number(card.dataset.rotateDeg || 0),
    flipX: Number(card.dataset.flipX || 0),
    flipY: Number(card.dataset.flipY || 0)
  };
};

const collectComicEditorState = () => {
  ...
  const cards = Array.from(document.querySelectorAll('#cm-pages-editor .comic-page-edit-card'));
  return {
    title,
    description,
    partId,
    locked,
    unlockAt,
    pages: cards.map((card, idx) => getComicPageDataFromCard(card, idx))
  };
};

const renderComicPublishChecklist = () => {
  const checklist = document.getElementById('cm-publish-checklist');
  if (!checklist) return;
  const state = collectComicEditorState();
  const issues = [];
  if (!state.title) issues.push('Add a comic title');
  if (!state.pages.length) issues.push('Add at least one page');
  const missingImages = state.pages.filter(p => !p.image).length;
  if (missingImages) issues.push(`${missingImages} page${missingImages > 1 ? 's' : ''} missing image`);
  const items = [
    { label: 'Title ready', ok: !!state.title },
    { label: 'At least one page', ok: state.pages.length > 0 },
    { label: 'All pages have images', ok: missingImages === 0 }
  ];
  checklist.innerHTML = items.map(item => `<div class="comic-checklist-item ${item.ok ? 'ok' : 'warn'}"><i data-lucide="${item.ok ? 'check-circle' : 'alert-circle'}"></i><span>${escapeHtml(item.label)}</span></div>`).join('');
  refreshLucide();
};

const saveComicDraft = () => {
  const state = collectComicEditorState();
  if (!state.title && !state.description && !state.pages.length) return;
  const draftState = {
    ...state,
    pages: state.pages.map(page => ({
      ...
      rotateDeg: page.rotateDeg,
      flipX: page.flipX,
      flipY: page.flipY
    }))
  };
  LS.set('comicDraftCurrent', draftState);
  const history = LS.get('comicDraftHistory') || [];
  history.unshift({ savedAt: Date.now(), state: draftState });
  LS.set('comicDraftHistory', history.slice(0, 5));
  const stamp = document.getElementById('cm-last-saved');
  if (stamp) stamp.textContent = `Saved ${new Date().toLocaleTimeString()}`;
  renderComicPublishChecklist();
};

const restoreComicDraft = (draft) => {
  if (!draft) return;
  document.getElementById('cm-title').value = draft.title || '';
  document.getElementById('cm-desc').value = draft.description || '';
  document.getElementById('cm-part').value = draft.partId || document.getElementById('cm-part').value;
  document.getElementById('cm-lock-toggle').checked = !!draft.locked;
  document.getElementById('cm-unlock-at').value = draft.unlockAt || '';
  const editor = document.getElementById('cm-pages-editor');
  if (editor) {
    editor.innerHTML = '';
    (draft.pages || []).forEach(page => addComicPageEditor(page));
  }
  renderComicPublishChecklist();
};

const processComicImageFile = (file, maxLongEdge = 1200) => {
...
const handleComicPageTool = async (btn, card) => {
  const action = btn.dataset.action;
  const hidden = card.querySelector('.cm-page-img-data');
  if (!hidden || !hidden.value) { toast('Upload an image before editing it.', 'warn'); return; }
  const current = hidden.value;
  const img = new Image();
  img.onload = () => {
    const canvas = document.createElement('canvas');
    const ctx = canvas.getContext('2d');
    const rotateDeg = Number(card.dataset.rotateDeg || 0);
    const flipX = Number(card.dataset.flipX || 0);
    const flipY = Number(card.dataset.flipY || 0);
    let nextAngle = rotateDeg;
    let nextFlipX = flipX;
    let nextFlipY = flipY;
    if (action === 'rotate') nextAngle = (rotateDeg + 90) % 360;
    if (action === 'flip-h') nextFlipX = flipX ? 0 : 1;
    if (action === 'flip-v') nextFlipY = flipY ? 0 : 1;
    if (action === 'mode-fit') card.dataset.displayMode = 'fit';
    if (action === 'mode-fill') card.dataset.displayMode = 'fill';
    if (action === 'mode-stretch') card.dataset.displayMode = 'stretch';
    card.dataset.rotateDeg = nextAngle;
    card.dataset.flipX = nextFlipX;
    card.dataset.flipY = nextFlipY;
    const width = img.width;
    const height = img.height;
    const radians = (nextAngle * Math.PI) / 180;
    const w = Math.abs(width * Math.cos(radians)) + Math.abs(height * Math.sin(radians));
    const h = Math.abs(width * Math.sin(radians)) + Math.abs(height * Math.cos(radians));
    ...
    updateComicPagePreviewFromData(card, hidden.value, card.querySelector('.cm-page-img-name')?.value || 'edited-image.jpg');
    renderComicPreviewFromCard(card);
    saveComicDraft();
  };
  img.src = current;
...
const readFileIntoCard = async (file, card) => {
  if (!file || !card) return;
  if (!(file.size <= 8 * 1024 * 1024)) { toast('File too large (max 8MB).', 'warn'); return; }
  const progressWrap = document.createElement('div');
  progressWrap.className = 'upload-progress';
  progressWrap.innerHTML = '<div class="bar"></div>';
  card.appendChild(progressWrap);
  const reader = new FileReader();
  reader.onprogress = (e) => {
    if (e.lengthComputable) {
      const pct = Math.round((e.loaded / e.total) * 100);
      const bar = progressWrap.querySelector('.bar'); if (bar) bar.style.width = pct + '%';
    }
  };
  reader.onload = async (e) => {
    try {
      const bar = progressWrap.querySelector('.bar'); if (bar) bar.style.width = '100%';
      setTimeout(() => progressWrap.remove(), 300);
      const dataUrl = await processComicImageFile(file, 1200);
      updateComicPagePreviewFromData(card, dataUrl, file.name);
      renderComicPreviewFromCard(card);
      saveComicDraft();
      renderComicPublishChecklist();
    } catch (err) {
      progressWrap.remove(); toast('Failed to read image file', 'warn');
    }
  };
  reader.onerror = () => { progressWrap.remove(); toast('Failed to read image file', 'warn'); };
  reader.readAsDataURL(file);
};
...
const handleComicUploadInput = (event) => {
  ...
  if (validFiles.length !== files.length) {
    toast('Some files exceeded 8MB and were skipped.', 'warn');
  }
  addComicPagesFromFiles(validFiles);
  event.target.value = '';
};

const addComicPagesFromFiles = (files) => {
  files.forEach((file) => {
    const card = addComicPageEditor({ image: '', imageName: file.name });
    readFileIntoCard(file, card);
  });
};

const enableComicPageReorder = () => {
  const container = document.getElementById('cm-pages-editor');
  if (!container) return;
  let dragEl = null;
  container.querySelectorAll('.comic-page-edit-card').forEach(card => {
    card.draggable = true;
    ...
    const dragstart = () => { dragEl = card; card.style.opacity = '0.5'; };
    const dragend = () => { if (dragEl) dragEl.style.opacity = ''; dragEl = null; reindexComicPages(); saveComicDraft(); };
    const dragover = (e) => { e.preventDefault(); };
    const drop = (e) => { e.preventDefault(); if (!dragEl || dragEl === card) return; container.insertBefore(dragEl, card.nextSibling); reindexComicPages(); saveComicDraft(); };
    ...
    card._dragstartHandler = dragstart;
    card._dragendHandler = dragend;
    card._dragoverHandler = dragover;
    card._dropHandler = drop;
  });
};

const handleComicPageFileInput = async (fileInput) => {
...
const openComicPageImageFull = (img) => {
  const src = img?.src || '';
  if (!src) return;
  const pageIndex = Number.isInteger(comicCurrentPage) ? comicCurrentPage : 0;
  const comicId = activeComic?.id || null;
  openFullscreenImageModal({ src, comicId, pageIndex });
};
...
const previewComicPageImage = (btn) => {
  ...
  openFullscreenImageModal({ src, pageIndex: 0 });
};
...
const duplicateComicPage = (btn) => {
  const card = btn.closest('.comic-page-edit-card');
  if (!card) return;
  const clone = card.cloneNode(true);
  clone.dataset.pageId = genUuid();
  card.insertAdjacentElement('afterend', clone);
  const newCard = clone;
  newCard.querySelectorAll('.cm-page-title, .cm-page-caption, .cm-page-img-name').forEach(input => {
    input.addEventListener('input', () => { saveComicDraft(); renderComicPreviewFromCard(newCard); renderComicPublishChecklist(); });
  });
  newCard.querySelectorAll('.comic-page-tools button').forEach(btnEl => {
    btnEl.addEventListener('click', () => handleComicPageTool(btnEl, newCard));
  });
  // Wire file input and choose button for the cloned card
  const fileInput = newCard.querySelector('.cm-page-file-input');
  const chooseBtn = newCard.querySelector('.cm-choose-file');
  const thumb = newCard.querySelector('.cm-page-thumb');
  if (chooseBtn && fileInput) {
    chooseBtn.addEventListener('click', (e) => { e.stopPropagation(); fileInput.click(); });
    fileInput.addEventListener('change', () => handleComicPageFileInput(fileInput));
  }
...
const saveComic = () => {
  ...
  const issues = [];
  if (!state.pages.length) issues.push('Add at least one page');
  if (state.pages.some(p => !p.image)) issues.push('Each page needs an image');
  if (issues.length) {
    toast(`Please fix: ${issues.join(', ')}`, 'warn');
    return;
  }
  ...
  if (comicModalEditId) {
    ...
    LS.set('comics', comics);
    console.debug('saveComic: updated comic saved locally', { currentUser: currentUser && { id: currentUser.id, role: currentUser.role }, comicsCount: (LS.get('comics') || []).length });
    // Remote upsert
    upsertTeacherRecord('comics', comics[idx]);
    ...
    LS.set('comics', comics);
    console.debug('saveComic: new comic saved locally', { currentUser: currentUser && { id: currentUser.id, role: currentUser.role }, comicsCount: (LS.get('comics') || []).length });
    // Remote insert/upsert
    upsertTeacherRecord('comics', newComic).then(remote => {
      if (remote && remote.id && remote.id !== newComic.id) {
        // update local id mapping if remote provided different id
        const list = LS.get('comics') || [];
        const i = list.findIndex(c => c.id === newComic.id);
        if (i !== -1) {
          list[i].id = remote.id;
          LS.set('comics', list);
        }
...
const deleteComic = (comicId) => {
  openConfirmModal('Are you sure you want to delete this comic?')
    .then(confirmed => {
      ...
      LS.set('comics', comics);
      // Remote delete
      deleteTeacherRecord('comics', comicId);
...
window.currentFullscreenComicPage = 0;

const updateFullscreenPageIndicator = () => {
  const indicator = document.getElementById('fullscreen-page-indicator');
  const comics = LS.get('comics') || [];
  const currentComic = comics.find(c => c.id === window.currentFullscreenComicId);
  if (!indicator) return;
  if (currentComic && Array.isArray(currentComic.pages) && currentComic.pages.length > 0) {
    indicator.textContent = `Page ${window.currentFullscreenComicPage + 1} of ${currentComic.pages.length}`;
  } else {
    indicator.textContent = '';
  }
};

const openFullscreenImageModal = (imageSrc, comicId = null, pageIndex = 0) => {
  // imageSrc may be string or object { src, comicId, pageIndex }
  ...
  if (typeof imageSrc === 'object' && imageSrc !== null) {
    ...
    window.currentFullscreenComicId = imageSrc.comicId || comicId || null;
    window.currentFullscreenComicPage = Number.isInteger(imageSrc.pageIndex) ? imageSrc.pageIndex : pageIndex;
  } else {
    img.src = imageSrc || '';
    window.currentFullscreenComicId = comicId || null;
    window.currentFullscreenComicPage = Number.isInteger(pageIndex) ? pageIndex : 0;
  }
...
const fullscreenNextComic = () => {
  ...
  const currentComic = comics.find(c => c.id === window.currentFullscreenComicId);
  if (currentComic && Array.isArray(currentComic.pages) && currentComic.pages.length > 0) {
    if (window.currentFullscreenComicPage < currentComic.pages.length - 1) {
      window.currentFullscreenComicPage += 1;
      const nextPage = currentComic.pages[window.currentFullscreenComicPage];
      openFullscreenImageModal({ src: nextPage.image || '', comicId: currentComic.id, pageIndex: window.currentFullscreenComicPage });
      return;
    }
  }
  ...
  openFullscreenImageModal({ src: nextImg, comicId: next.id, pageIndex: 0 });
};
...
const fullscreenPrevComic = () => {
  ...
  const currentComic = comics.find(c => c.id === window.currentFullscreenComicId);
  if (currentComic && Array.isArray(currentComic.pages) && currentComic.pages.length > 0) {
    if (window.currentFullscreenComicPage > 0) {
      window.currentFullscreenComicPage -= 1;
      const prevPage = currentComic.pages[window.currentFullscreenComicPage];
      openFullscreenImageModal({ src: prevPage.image || '', comicId: currentComic.id, pageIndex: window.currentFullscreenComicPage });
      return;
    }
  }
  const idx = comics.findIndex(c => c.id === window.currentFullscreenComicId);
  ...
  openFullscreenImageModal({ src: prevImg, comicId: prev.id, pageIndex: 0 });
};
...
const renderComics = () => {
  ...
  const comics = LS.get('comics') || [];
  console.debug('renderComics: currentUser', currentUser && { id: currentUser.id, role: currentUser.role }, 'comicsCount', comics.length);
  const parts = getStoredParts();
...
const openComicReader = (comicId) => {
  ...
  // Load saved progress if we're a student
  if (currentUser && currentUser.role === 'student') {
    const progressKey = `comic_progress_${currentUser.id}_${comicId}`;
    const savedProgress = LS.get(progressKey);
    if (savedProgress && savedProgress.currentPage !== undefined) {
      comicCurrentPage = savedProgress.currentPage;
      if (comicCurrentPage > 0) {
        toast(`Resuming at page ${comicCurrentPage + 1}`, 'info');
      }
    } else {
      comicCurrentPage = 0;
    }
...
const saveComicProgress = () => {
  if (!currentUser || currentUser.role !== 'student' || !activeComic) return;
  
  const progressKey = `comic_progress_${currentUser.id}_${activeComic.id}`;
  const progress = {
    currentPage: comicCurrentPage,
    totalPages: activeComic.pages.length,
    readAt: Date.now()
  };
...
const renderComicPage = () => {
  ...
  // Update progress bar
  const progressPercent = ((comicCurrentPage + 1) / activeComic.pages.length) * 100;
  const progressContainer = document.getElementById('cr-progress-container');
  const progressFill = document.getElementById('cr-progress-fill');
  ...
  // If page has an image, show image-only view (hide text area and expand image container)
  const textContainer = document.getElementById('cr-panel-text');
  const imageContainer = document.getElementById('cr-panel-image-container');
  if (page.image) {
    if (textContainer) textContainer.style.display = 'none';
    if (imageContainer) {
      imageContainer.style.height = 'calc(100vh - 160px)'; // Adjust for progress bar
    ...
    if (imageContainer) {
      imageContainer.style.height = 'calc(100vh - 280px)'; // Adjust for progress bar
  ...
  // Save progress after rendering page
  saveComicProgress();
...
const nextComicPage = () => {
  if (comicCurrentPage < activeComic.pages.length - 1) {
    ...
    if (currentUser && currentUser.role === 'student') {
      ...
      // Mark comic as completely read
      const progressKey = `comic_progress_${currentUser.id}_${activeComic.id}`;
      LS.set(progressKey, {
        currentPage: activeComic.pages.length - 1,
        totalPages: activeComic.pages.length,
        completed: true,
        readAt: Date.now()
      });
...
const openQuizDetailsModal = () => {
  ...
  if (qzPartSelect) {
    qzPartSelect.innerHTML = parts.map((p, idx) => `<option value="${p.id}">${p.title || getPartOrdinalLabel(idx)}</option>`).join('');
...
const updateQuizLockUI = () => {
  ...
  if (icon && button && unlockAtInput) {
    const unlockRow = document.getElementById('qz-unlock-row');
    if (quizIsLocked) {
      ...
      if (label) { label.textContent = 'Lock This Quiz'; label.style.color = 'var(--txt3)'; }
...
const renderQuizReports = () => {
  ...
  const participationRate = students.length > 0 ? Math.round((studentsWithAttempts / students.length) * 100) : 0;
  const averageAttemptsPerStudent = studentsWithAttempts > 0 ? Math.round(totalQuizAttempts / studentsWithAttempts) : 0;
  
  const scoreBuckets = [0, 0, 0, 0, 0, 0];
  attempts.forEach(att => {
    const pct = att.totalQuestions > 0 ? Math.round((att.score / att.totalQuestions) * 100) : 0;
    const bucket = Math.min(5, Math.floor(pct / 20));
    scoreBuckets[bucket]++;
  });
  const maxScoreBucket = Math.max(...scoreBuckets, 1);
  
  const attemptsByQuiz = quizzes.map(q => {
    const qAttempts = attempts.filter(a => a.quizId === q.id);
    return {
      title: q.title,
  ...
  const maxAttempts = Math.max(...attemptsByQuiz.map(item => item.count), 1);
  ...
  container.innerHTML = `
    ...
    <div class="analytics-grid">
      <div class="analytics-card">
        <div class="analytics-card-title">Students Registered</div>
        <div class="analytics-card-value">${students.length}</div>
        <div class="analytics-card-sub">Total students in this class</div>
      </div>
      <div class="analytics-card">
        <div class="analytics-card-title">Total Quiz Attempts</div>
        <div class="analytics-card-value">${totalQuizAttempts}</div>
        <div class="analytics-card-sub">Attempts submitted across quizzes</div>
      </div>
      <div class="analytics-card">
        <div class="analytics-card-title">Average Score</div>
        <div class="analytics-card-value">${overallAvgScore}%</div>
        <div class="analytics-card-sub">Score across all quiz attempts</div>
      </div>
      <div class="analytics-card">
        <div class="analytics-card-title">Participation Rate</div>
        <div class="analytics-card-value">${participationRate}%</div>
        <div class="analytics-card-sub">Students who attempted at least one quiz</div>
      </div>
    </div>

    <div class="report-graphs-grid">
      <div class="rcard2 graph-card">
        <h4 class="rcard2-title">Quiz Engagement Trend</h4>
        <div class="bar-chart">
          ${attemptsByQuiz.map(item => {
            const width = Math.max(12, Math.round((item.count / maxAttempts) * 100));
            return `
              <div class="bar-row">
      ...
      <div class="rcard2 graph-card">
        <h4 class="rcard2-title">Score Distribution</h4>
        <div class="score-dist">
          ${['0-19','20-39','40-59','60-79','80-99','100'].map((label, idx) => {
            const count = scoreBuckets[idx];
      ...
      <div class="rcard2 graph-card gauge-card">
        <h4 class="rcard2-title">Completion vs Participation</h4>
        <div class="report-gauge-row">
          <div class="report-gauge" style="background: conic-gradient(var(--leaf) 0 ${completionRate}%, var(--surf2) ${completionRate}% 100%);">
          ...
          <div class="gauge-stats">
            <div class="gauge-stat">
              <div class="gauge-stat-label">Participation</div>
              <div class="gauge-stat-value">${participationRate}%</div>
            </div>
            <div class="gauge-stat">
              <div class="gauge-stat-label">Avg Attempts</div>
              <div class="gauge-stat-value">${averageAttemptsPerStudent}</div>
            </div>
          </div>
        </div>
      </div>
  ...
  grid.innerHTML = quizzes.map(q => {
    ...
    const avgScore = qAttempts.length > 0 && q.type !== 'Survey Questionnaire'
      ...
      : 0;
    const quizTakeCount = qAttempts.length;
    const scoreLabel = q.type !== 'Survey Questionnaire' ? `${avgScore}%` : `${quizTakeCount} responses`;

    return `
      <div class="report-quiz-card" onclick="viewQuizReportDetails('${q.id}')">
        <div style="display:flex;justify-content:space-between;align-items:flex-start;">
          <h4 class="report-quiz-title">${escapeHtml(q.title)}</h4>
          <span class="cbadge report-quiz-type" style="background:${q.type === 'Survey Questionnaire' ? 'var(--sky)' : 'var(--amber)'};color:var(--bg);">${escapeHtml(q.type)}</span>
        </div>
        <div style="margin-top:12px;font-size:0.8rem;color:var(--txt2);">
          <div>Taken by: <strong>${quizTakeCount}</strong> students</div>
          <div>${scoreLabel}</div>
        </div>
...
const reviewStudentProgress = (studentId) => {
  ...
  const allGameAttempts = LS.get('game_attempts') || [];
  const games = LS.get('games') || [];
  const studentAttempts = allAttempts.filter(a => a.studentId === studentId);
  const studentGameAttempts = allGameAttempts.filter(a => a.studentId === studentId);
  ...
  // Student info section with game stats
  ...
  const completedGameAttempts = studentGameAttempts.filter(a => a.completed);
  const avgGameTime = completedGameAttempts.length > 0
    ? Math.round(completedGameAttempts.reduce((sum, a) => sum + (a.timeUsedSeconds || 0), 0) / completedGameAttempts.length)
    : 0;
    ...
    infoEl.innerHTML = `
      <div style="display:flex;align-items:center;gap:14px;">
        ...
        <div>
          ...
          <div style="font-size:0.82rem;color:var(--txt2);margin-top:4px;">
            ${studentAttempts.length} quiz attempt(s) • ${studentGameAttempts.length} game attempt(s) • ${completedGameAttempts.length} game(s) completed
            ${avgGameTime > 0 ? ` • Avg game time: ${formatTime(avgGameTime)}` : ''}
            ${student.blocked ? ' <span style="color:#f87171;font-weight:600;">[BLOCKED]</span>' : ''}
          </div>
        </div>
      </div>
    `;
  ...
  // Attempts list section (combine quiz and game attempts)
  const listEl = document.getElementById('student-review-attempts-list');
  if (!listEl) return;
  
  if (studentAttempts.length === 0 && studentGameAttempts.length === 0) {
    listEl.innerHTML = `<div style="text-align:center;padding:32px;color:var(--txt3);">This student has not taken any quizzes or played any games yet.</div>`;
  } else {
    // Build quiz attempts HTML
    const quizAttemptsHtml = studentAttempts.map((attempt, aIdx) => {
      ...
      const completedAt = attempt.completedAt ? new Date(attempt.completedAt).toLocaleString() : 'Not completed';
      ...
      const cardId = `quiz-attempt-card-${aIdx}`;
      const detailId = `quiz-attempt-detail-${aIdx}`;
    ...
    // Build game attempts HTML
    const gameAttemptsHtml = studentGameAttempts.map((attempt, aIdx) => {
      const game = games.find(g => g.id === attempt.gameId);
      const gameTitle = game ? game.title : 'Unknown Game';
      const gameType = attempt.gameType || 'Game';
      const completedAt = attempt.completedAt ? new Date(attempt.completedAt).toLocaleString() : (attempt.startedAt ? new Date(attempt.startedAt).toLocaleString() : 'Not started');
      
      const statusLabel = attempt.completed ? 'Completed' : (attempt.timeExpired ? 'Time Expired' : 'In Progress');
      const statusColor = attempt.completed ? 'var(--leaf)' : (attempt.timeExpired ? '#f87171' : 'var(--sky)');
      const typeIcon = '🎮';
      const timeText = attempt.timeUsedSeconds ? formatTime(attempt.timeUsedSeconds) : '—';
      const cardId = `game-attempt-card-${aIdx}`;
      ...
      return `
        <div style="background:var(--surf);border:1px solid var(--bdr2);border-radius:14px;overflow:hidden;">
          ...
          <div style="display:flex;align-items:center;gap:14px;padding:14px 16px;">
            ...
            <!-- Text -->
            <div style="flex:1;min-width:0;">
              <div style="font-weight:700;font-size:0.92rem;color:var(--txth);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${gameTitle}</div>
              <div style="font-size:0.77rem;color:var(--txt3);margin-top:2px;">${gameType} &bull; ${completedAt}</div>
            </div>
            <!-- Score badge + chevron -->
            <div style="display:flex;align-items:center;gap:10px;flex-shrink:0;">
              <div style="text-align:right;">
                <div style="font-size:0.88rem;font-weight:700;color:var(--txth);">${timeText}</div>
                <div style="font-size:0.72rem;font-weight:600;color:${statusColor};">${statusLabel}</div>
              </div>
            </div>
          </div>
    ...
    listEl.innerHTML = [
      studentAttempts.length > 0 ? `<div style="font-weight:700;color:var(--txth);margin-bottom:12px;margin-top:8px;">📝 Quizzes</div>${quizAttemptsHtml}` : '',
      studentGameAttempts.length > 0 ? `<div style="font-weight:700;color:var(--txth);margin-bottom:12px;margin-top:24px;">🎮 Games</div>${gameAttemptsHtml}` : ''
    ].join('');
  }
...
const toggleRoomDropdown = (e) => {
  ...
  if (!menu) {
    menu = document.getElementById('roomDropdownMenu') || document.getElementById('teacherRoomDropdownMenu');
  }
  const chevron = container ? container.querySelector('.dropdown-chevron') : document.querySelector('.dropdown-chevron');
  if (menu) {
  ...
  if (btn && btn.id === 'sidebar-btn-room-teacher') {
    setTeacherTab('room');
  } else {
    setStudentTab('room');
  }
...
document.addEventListener('click', (e) => {
  try {
    ...
    // room dropdowns for teacher and student
    const roomMenus = Array.from(document.querySelectorAll('#roomDropdownMenu, #teacherRoomDropdownMenu'));
    roomMenus.forEach(menu => {
      if (!menu) return;
      const container = menu.closest('.sidebar-dropdown-container');
      const btn = container ? container.querySelector('.dropdown-toggle') : null;
...
// Process an image file: crop to square center and resize to maxSize (px)
const processImageFile = (file, maxSize = 512) => {
  return new Promise((resolve, reject) => {
    const reader = new FileReader();
    reader.onload = (e) => {
      const img = new Image();
...
const uploadAvatarDataUrl = (dataUrl, onProgress) => {
  return new Promise((resolve, reject) => {
    try {
...
const updateUserPhotoLocal = (dataUrl) => {
  // Update current user and lists and rooms
  currentUser.photoURL = dataUrl;
  LS.set('currentUser', currentUser);
  if (currentUser.role === 'teacher') {
    const teachers = LS.get('teachers') || [];
    const idx = teachers.findIndex(t => t.id === currentUser.id);
    if (idx !== -1) { teachers[idx].photoURL = dataUrl; LS.set('teachers', teachers); }
  } else {
    const students = LS.get('students') || [];
    const idx = students.findIndex(s => s.id === currentUser.id);
    if (idx !== -1) { students[idx].photoURL = dataUrl; LS.set('students', students); }
  }
  const rooms = LS.get('rooms') || {};
  let changed = false;
  for (let code in rooms) {
    const r = rooms[code];
    if (r && Array.isArray(r.joinedStudents)) {
      const studentIdx = r.joinedStudents.findIndex(s => s.id === currentUser.id);
      if (studentIdx !== -1) { r.joinedStudents[studentIdx].photoURL = dataUrl; changed = true; }
    }
    if (Array.isArray(r.chatMessages)) {
      r.chatMessages.forEach(msg => { if (msg.senderId === currentUser.id) { msg.photoURL = dataUrl; changed = true; } });
    }
  }
  if (changed) LS.set('rooms', rooms);
  // re-render
  openUserProfile(); populateDashboard(); if (currentUser.role === 'student') refreshStudentRoomUI(); else refreshTeacherRoomUI();
};

const handleProfilePicChange = async (inputOrFile) => {
  let file = null;
  if (inputOrFile instanceof Element && inputOrFile.files) file = inputOrFile.files[0];
  else file = inputOrFile;
  if (!file) return;
  try {
    const dataUrl = await processImageFile(file, 512);
    // preview immediately
    const imgEl = document.querySelector('.profile-pic-img');
    ...
    if (progWrap && progBar) { progWrap.classList.remove('hidden'); progBar.style.width = '4%'; }

    // Try server upload, fallback to local storage
    try {
      const finalUrl = await uploadAvatarDataUrl(dataUrl, (pct) => { if (progBar) progBar.style.width = pct + '%'; });
      updateUserPhotoLocal(finalUrl);
      if (progBar) progBar.style.width = '100%';
      setTimeout(() => { if (progWrap) progWrap.classList.add('hidden'); }, 600);
      toast('Profile picture updated', 'ok');
    } catch (err) {
      // fallback: save locally
      updateUserPhotoLocal(dataUrl);
      if (progWrap) setTimeout(() => progWrap.classList.add('hidden'), 400);
      toast('Saved locally (upload failed)', 'warn');
    }
  } catch (err) {
    console.error('handleProfilePicChange error', err);
    toast('Unable to process image', 'warn');
  }
};
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  addEnterKeyHandler('topSearch', handleTopSearch);
  try { setupTopSearch(); } catch (e) {}

  // Initialize basic accessibility features
  try { initAccessibility(); } catch (e) { console.warn('initAccessibility failed', e); }
...
// Top navbar search handler (simple client-side search placeholder)
const handleTopSearch = () => {
  const q = (document.getElementById('topSearch') || {}).value || '';
  const term = String(q).trim();
  if (!term) return;
  // Simple search: look in quizzes, comics, and students and show quick toast with counts
  const quizzes = LS.get('quizzes') || [];
  const comics = LS.get('comics') || [];
  const students = LS.get('students') || [];
  const qCount = quizzes.filter(z => (z.title || '').toLowerCase().includes(term.toLowerCase())).length;
  const cCount = comics.filter(z => (z.title || '').toLowerCase().includes(term.toLowerCase())).length;
  const sCount = students.filter(z => ((z.fullName||'') + ' ' + (z.username||'')).toLowerCase().includes(term.toLowerCase())).length;
  toast(`Search results — quizzes: ${qCount}, comics: ${cCount}, students: ${sCount}`, 'ok');
};
...
const setupTopSearch = () => {
  ...
  input.addEventListener('input', (e) => {
    ...
    timer = setTimeout(() => renderSearchSuggestions(val), 180);
  });
  input.addEventListener('keydown', (e) => {
    if (e.key === 'Escape') { hideSearchSuggestions(); closeMobileSearch(); }
  });
};

const renderSearchSuggestions = (term) => {
  ...
  // parts
  parts.filter(p => (p.title||'').toLowerCase().includes(lc)).slice(0,4).forEach(p => out.push({type:'part', id:p.id, title:p.title || 'Part', sub:'Part'}));
  // quizzes
  quizzes.filter(z => (z.title||'').toLowerCase().includes(lc)).slice(0,6).forEach(z => out.push({type:'quiz', id:z.id, title:z.title, sub:'Quiz'}));
  // comics
  comics.filter(z => (z.title||'').toLowerCase().includes(lc)).slice(0,4).forEach(z => out.push({type:'comic', id:z.id, title:z.title, sub:'Comic'}));
  // students
  students.filter(s => (((s.fullName||'') + ' ' + (s.username||'')).toLowerCase().includes(lc))).slice(0,6).forEach(s => out.push({type:'student', id:s.id, title:(s.fullName||s.firstName||s.username), sub: s.username || ''}));

  if (out.length === 0) { hideSearchSuggestions(); return; }
  container.innerHTML = out.map(item => `
    <div class="sugg-item" data-type="${item.type}" data-id="${item.id}" onclick="selectSearchResult('${item.type}','${item.id}')">
      <div class="sugg-type">${item.sub}</div>
      <div class="sugg-title">${escapeHtml(item.title || '-')}</div>
      <div class="sugg-sub">${item.type}</div>
    </div>
  `).join('');
  container.classList.remove('hidden');
};
...
// Setup handlers for profile image area: drag/drop, paste, edit/remove buttons
const setupProfileImageHandlers = () => {
  const uploader = document.getElementById('profile-pic-uploader');
  const editBtn = document.getElementById('profileEditBtn');
  const removeBtn = document.getElementById('profileRemoveBtn');
  const picArea = document.getElementById('profilePicArea') || document.getElementById('profilePicContainer');

  if (editBtn && uploader) editBtn.addEventListener('click', (e) => { e.stopPropagation(); uploader.click(); });
  if (uploader) uploader.addEventListener('change', (e) => handleProfilePicChange(uploader));
  if (removeBtn) removeBtn.addEventListener('click', (e) => { e.stopPropagation(); removeAvatar(); });

  if (picArea) {
    picArea.addEventListener('dragover', (e) => { e.preventDefault(); picArea.classList.add('dragover'); });
    picArea.addEventListener('dragleave', (e) => { e.preventDefault(); picArea.classList.remove('dragover'); });
    picArea.addEventListener('drop', (e) => {
      e.preventDefault(); picArea.classList.remove('dragover');
      const f = e.dataTransfer && e.dataTransfer.files && e.dataTransfer.files[0];
      if (f) handleProfilePicChange(f);
    });
    // paste support
    picArea.addEventListener('paste', (e) => {
      const items = (e.clipboardData && e.clipboardData.items) || [];
      for (let i=0;i<items.length;i++) {
        const it = items[i];
        if (it.type && it.type.indexOf('image') === 0) {
          const blob = it.getAsFile();
          if (blob) handleProfilePicChange(blob);
          e.preventDefault();
          return;
        }
      }
...
const removeAvatar = () => {
  if (!currentUser) return;
  const confirmed = confirm('Remove profile photo? This cannot be undone.');
  if (!confirmed) return;
  // store for undo
  window._lastRemovedAvatar = currentUser.photoURL || null;
  // clear
  currentUser.photoURL = null; LS.set('currentUser', currentUser);
  if (currentUser.role === 'teacher') {
    const teachers = LS.get('teachers') || []; const idx = teachers.findIndex(t=>t.id===currentUser.id); if (idx!==-1) { delete teachers[idx].photoURL; LS.set('teachers', teachers); }
  } else {
    const students = LS.get('students') || []; const idx = students.findIndex(s=>s.id===currentUser.id); if (idx!==-1) { delete students[idx].photoURL; LS.set('students', students); }
  }
  const rooms = LS.get('rooms') || {}; for (let code in rooms) { const r = rooms[code]; if (r && Array.isArray(r.joinedStudents)) { const sidx = r.joinedStudents.findIndex(s=>s.id===currentUser.id); if (sidx!==-1) { delete r.joinedStudents[sidx].photoURL; } } if (Array.isArray(r.chatMessages)) { r.chatMessages.forEach(msg=>{ if (msg.senderId===currentUser.id) delete msg.photoURL; }); } } LS.set('rooms', rooms);
  populateDashboard(); openUserProfile();
  toast('Profile photo removed. Undo?', 'warn');
};

const undoRemoveAvatar = () => {
  if (!window._lastRemovedAvatar) return;
  handleProfilePicChange(dataURLtoFile(window._lastRemovedAvatar, 'restored.jpg'));
  window._lastRemovedAvatar = null;
};

// Helpers to convert dataURL to File for reuse
...
const selectSearchResult = (type, id) => {
  hideSearchSuggestions();
  const term = document.getElementById('topSearch');
  if (term) term.value = '';
  // Navigate to appropriate tab and attempt to focus item
  if (type === 'quiz') {
    setTeacherTab('quizzes');
    // try to scroll to quiz element if present
    setTimeout(() => {
...
const openMobileSearch = () => {
  ...
  setTimeout(() => {
    const input = document.getElementById('topSearch'); if (input) input.focus();
  }, 60);
};
